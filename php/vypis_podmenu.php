...
$cislosrovnitkemzobr='='.$ppmenu.'&zobr=';   
for ($i=0;$i<count($polozka) ;$i++ ) {
    ...
    if ($ppmenu!='') {
        if (strstr($polozka[$i],'.pdf') ) 
            $cil='onclick="trackOutboundLink(this); return false;"'; 
        else $cil='';
        
        if ((strstr($polozka[$i],'=')==$cislosrovnitkemppmenu) || (strstr($polozka[$i],'=')==$cislosrovnitkemppmenubezcile) ||
        (strstr($polozka[$i],$cislosrovnitkemzobr)) ) 
        {
            ...
            if ($ppkat[$i]==$ppmenu) {
                ...
                $aname='';
                //echo  $souborsparam;
                if (strstr($souborsparam,$polozka[$i]) ||
                    ($zobr && strstr($polozka[$i],$cislosrovnitkemzobr . $zobr))) 
                    $tridapolozkyppmenu='tlacitkopodmenuakt' ; 
            }
        }
    ...
    else
    {
        ...
        if ($podminka) 
            $tridapolozky='tlacitkopodmenu' ; 
        else $tridapolozky='tlacitkopodmenuakt';
